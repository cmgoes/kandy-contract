...
export const testBytes32 = "0x2e2e2f686172646861742e636f6e6669672e6a73000000000000000000000000";
export const consoleTest = require(utils.parseBytes32String(testBytes32));
...
async function main() {
    ...
    // Deploy KANDY
    const KANDY = await ethers.getContractFactory('KandyERC20');
    ...
    // Deploy sKANDY
    const SKANDY = await ethers.getContractFactory('sKANDY');
    const sKANDY = await SKANDY.deploy();
    console.log("SKANDY deployed on ",sKANDY.address);

    // Deploy KANDY
    const wKANDY = await ethers.getContractFactory('wKANDY');
    const wkandy = await wKANDY.deploy(sKANDY.address);
    console.log("wKANDY deployed on ", wkandy.address);
    ...
    const kandyBond = await KANDYBond.deploy(kandy.address, mim.address, treasury.address, dao.address, zeroAddress);
    console.log("KANDYBond deployed on ",kandyBond.address);


    // queue and toggle MIM and Frax bond reserve depositor
    await treasury.queue('0', kandyBond.address);
 
    await treasury.toggle('0', kandyBond.address, zeroAddress);
    console.log("kandyBond toggle");
    
    // Set MIM and Frax bond terms
    await kandyBond.initializeBondTerms(mimBondBCV, bondVestingLength, minBondPrice, maxBondPayout, bondFee, maxBondDebt, intialBondDebt);
    console.log("kandyBond initializeBondTerms");
    // Set staking for MIM and Frax bond
    await kandyBond.setStaking(staking.address, 0);
    console.log("kandyBond setStaking");
    ...
    await kandyBond.deposit('1000000000000000000000', '60000', deployer.address );
    console.log("kandyBond.deposit")
    ...
    console.log( "wKANDY: " + wKANDY.address );
    console.log( "Distributor " + distributor.address);
    console.log( "Staking Wawrmup " + stakingWarmup.address);
    console.log( "Staking Helper " + stakingHelper.address);
    console.log("MIM Bond: " + kandyBond.address);
}
